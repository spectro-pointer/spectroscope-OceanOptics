...
function currentspectrum($channel = 0) {
	...
	global $ip_address, $port, $cmd_get_current_spectrum;

    $socket = connect_or_abort($ip_address, $port);

    $arguments = $channel . ";";
    $msg = $cmd_get_current_spectrum . string_length_bytes($arguments);
    $msg = $msg . $arguments;
	...
	$result = socket_read_n($socket, $spectrum_string, $result_length);
	socket_close($socket);
