...
function setcalibrationbuffer($calibration, $channel = 0) {
	global $ip_address, $port, $cmd_set_calibration_coefficients_to_buffer;

	$socket = connect_or_abort($ip_address, $port);

    $arguments = $channel . ";" . $calibration;
    $msg = $cmd_set_calibration_coefficients_to_buffer . string_length_bytes($arguments);
    $msg = $msg . $arguments;
