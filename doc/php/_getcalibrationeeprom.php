...
function getcalibrationeeprom($channel = 0) {
	global $ip_address, $port, $cmd_get_calibration_coefficients_from_eeprom;

	$socket = connect_or_abort($ip_address, $port);

    $arguments = $channel . ";";
    $msg = $cmd_get_calibration_coefficients_from_eeprom . string_length_bytes($arguments);
    $msg = $msg . $arguments;
