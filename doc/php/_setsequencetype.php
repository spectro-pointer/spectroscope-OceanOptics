...
function setsequencetype($type, $channel = 0) {
	global $ip_address, $port, $cmd_set_sequence_type;

	$socket = connect_or_abort($ip_address, $port);

    $arguments = $channel . ";" . $type;
	$msg = $cmd_set_sequence_type . string_length_bytes($arguments);
	$msg = $msg . $arguments;
