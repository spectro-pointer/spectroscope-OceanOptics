...
function getsequencenumber($channel = 0) {

	global $ip_address, $port, $cmd_get_current_sequence_number;

	$socket = connect_or_abort($ip_address, $port);

    $arguments = $channel . ";";
    $msg = $cmd_get_current_sequence_number . string_length_bytes($arguments);
    $msg = $msg . $arguments;
